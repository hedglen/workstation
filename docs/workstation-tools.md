...
| **Windows Terminal** / **WezTerm**      | **WezTerm** (`dotfiles/wezterm/wezterm.lua`): system tab (drives, IP, update runbook in the right pane); **toolbelt** tab (Scoop CLI cheat sheet in the right pane); **git** tab (workspace-folder clean/dirty + commit checklist, refreshes); **wsl** tab with right-pane helper (`wezterm/wsl-helper.sh`); dedicated **claude**, **codex**, and **grok** tabs. **Windows Terminal**: linked `dotfiles/windows-terminal/settings.json`. |
